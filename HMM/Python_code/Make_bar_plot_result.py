...
plt.show()
###########Estimated data
data=pd.read_csv("HMM/data/Estimated_data_through_HMM.csv")
or_s=data['State']
obs=data['Observations']
obs0=[0,0,0,0]
obs1=[0,0,0,0]
...
plt.rc('axes', labelsize=22)
plt.rc('axes',titlesize=22)
plt.bar(obs_S0[0:len(obs_S0)],obsS1[0:len(obsS1)], color='red', width=barWidth,label="CA1")
plt.bar(obs_S1[0:len(obs_S1)],obsS2[0:len(obsS2)], color='green', width=barWidth,label="CA2")
plt.bar(obs_S2[0:len(obs_S2)],obsS3[0:len(obsS3)], color='blue', width=barWidth,label="CA3")
plt.bar(obs_S3[0:len(obs_S3)],obsS4[0:len(obsS4)], color='purple', width=barWidth,label="CA4")
plt.xticks(fontsize=20)
plt.yticks(fontsize=20)
